...
> **"共享"不等于"不更新"**
>
> 参数共享的意思是：**在一次前向传播中**，处理 $x_1, x_2, \ldots, x_T$ 时用的是同一个 $W_h$。但训练时 $W_h$ 照样被梯度更新——每个时间步都贡献一份梯度，反向传播时把所有时间步的梯度**累加**起来：
>
> $$\frac{\partial \mathcal{L}}{\partial W_h} = \sum_{t=1}^{T} \frac{\partial \mathcal{L}}{\partial h_t} \cdot \frac{\partial h_t}{\partial W_h}$$
>
> 然后执行 $W_h \leftarrow W_h - \eta \cdot \frac{\partial \mathcal{L}}{\partial W_h}$，下一个 batch 就用更新后的 $W_h$。这和 CNN 的卷积核完全一样：卷积核在图上滑动 100 个位置时用同一个核，但每个 batch 后照样根据梯度更新。
>
> **与 MLP 的对比**：MLP 每层有不同的 $W_1, W_2, \ldots$，各自独立更新。RNN 只有一个 $W_h$，被所有时间步共用、被所有时间步的梯度共同更新。这也是为什么 RNN 能处理变长序列——参数量不随序列长度变化。
...
| 公式 | 含义 |
|------|------|
| $\mathbf{h}_t = \tanh(W_h \mathbf{h}_{t-1} + W_x \mathbf{x}_t + \mathbf{b})$ | RNN 隐状态更新 |
| $\frac{\partial \mathcal{L}_T}{\partial \mathbf{h}_1} \propto \prod_{t=2}^{T} \text{diag}(\tanh'(\cdot)) \cdot W_h$ | BPTT 梯度链：$W_h$ 连乘导致消失/爆炸 |
| $\mathbf{f}_t = \sigma(W_f [\mathbf{h}_{t-1}, \mathbf{x}_t] + \mathbf{b}_f)$ | LSTM 遗忘门 |
| $\mathbf{i}_t = \sigma(W_i [\mathbf{h}_{t-1}, \mathbf{x}_t] + \mathbf{b}_i)$ | LSTM 输入门 |
| $\tilde{\mathbf{c}}_t = \tanh(W_c [\mathbf{h}_{t-1}, \mathbf{x}_t] + \mathbf{b}_c)$ | LSTM 候选细胞状态 |
| $\mathbf{c}_t = \mathbf{f}_t \odot \mathbf{c}_{t-1} + \mathbf{i}_t \odot \tilde{\mathbf{c}}_t$ | LSTM 细胞状态更新（加法路径） |
| $\mathbf{o}_t = \sigma(W_o [\mathbf{h}_{t-1}, \mathbf{x}_t] + \mathbf{b}_o)$ | LSTM 输出门 |
| $\mathbf{h}_t = \mathbf{o}_t \odot \tanh(\mathbf{c}_t)$ | LSTM 隐状态输出 |
| $\mathbf{h}_t = (1 - \mathbf{z}_t) \odot \mathbf{h}_{t-1} + \mathbf{z}_t \odot \tilde{\mathbf{h}}_t$ | GRU 隐状态更新（更新门 $\mathbf{z}_t$） |
...
> 提示：代入公式 $c_t = f_t \odot c_{t-1} + i_t \odot \tilde{c}_t$，注意遗忘门的名字和它的值的含义

你的回答：


**Q3**：RNN 必须串行计算是因为"$\mathbf{h}_t$ 依赖 $\mathbf{h}_{t-1}$"。Transformer 为什么不需要串行？它用什么机制替代了"逐步传递信息"这个过程？如果 Transformer 也需要知道位置信息（第 3 个词 vs 第 100 个词），它是怎么做到的？（提示：回忆位置编码 positional encoding）
