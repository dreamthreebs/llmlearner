...
**只换激活函数不够**：即使用了 ReLU（导数 = 1），如果权重尺度不对（比如都初始化为很大的值），$W_l$ 这一项仍然会导致爆炸。这就是为什么 ReLU 配 He 初始化、残差连接配 LayerNorm 是"成套出现"的——它们解决的是梯度传播中不同环节的问题。
...
**Q1**：假设你有一个 3 层 MLP，每层用 Sigmoid 激活函数，输入是 $x = 10$。请估算第一层的梯度大约会缩小多少倍。（提示：$x = 10$ 时 $\sigma(10)$ 接近多少？$\sigma'(10)$ 接近多少？）

你的回答：



**Q2**：ReLU 在 $x = 0$ 处不可导。为什么这在实践中不是问题？（提示：想想训练数据中 $x$ 精确等于 0 的概率。）

你的回答：



**Q3**：GELU 和 ReLU 的核心区别是什么？为什么 Transformer 倾向于用 GELU 而不是 ReLU？（提示：想想 Transformer 的 FFN 层对平滑性的需求。）
