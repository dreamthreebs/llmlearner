...
| 公式 | 含义 |
|------|------|
| $A_{ij} = 1 \Leftrightarrow (v_i, v_j) \in E$ | 邻接矩阵 |
| $D_{ii} = \sum_j A_{ij}$ | 度矩阵（对角元为节点度） |
| $\tilde{A} = A + I$ | 加自环的邻接矩阵 |
| $\tilde{D}^{-1/2} \tilde{A} \tilde{D}^{-1/2}$ | 对称归一化（防止高度节点淹没低度节点） |
| $H^{(k+1)} = \sigma(\tilde{D}^{-1/2} \tilde{A} \tilde{D}^{-1/2} H^{(k)} W^{(k)})$ | GCN 层公式（矩阵形式） |
| $h_i^{(k+1)} = \sigma\left(\sum_{j \in \mathcal{N}(i) \cup \{i\}} \frac{1}{\sqrt{\tilde{d}_i \tilde{d}_j}} h_j^{(k)} W^{(k)}\right)$ | GCN 单节点聚合 |
| $e_{ij} = \text{LeakyReLU}(\mathbf{a}^T [Wh_i \| Wh_j])$ | GAT 注意力系数 |
| $\alpha_{ij} = \text{softmax}_j(e_{ij})$ | GAT 注意力权重（邻居上归一化） |
| $h_i' = \sigma\left(\sum_{j \in \mathcal{N}(i)} \alpha_{ij} Wh_j\right)$ | GAT 加权聚合 |

**常用数值**：大多数 GNN 只用 2–4 层（过平滑限制）。

---

## 理解检测

**Q1**：GCN 的层公式中用了 $\tilde{A} = A + I$（给每个节点加自环）。如果不加这个自环（直接用 $A$ 而不是 $\tilde{A}$），在消息传递过程中会发生什么？节点会丢失什么信息？

> 提示：用 GCN 层公式 $H^{(k+1)} = \sigma(\tilde{D}^{-1/2} \tilde{A} \tilde{D}^{-1/2} H^{(k)} W^{(k)})$，或聚合 $AH$ 的第 $i$ 行含义。
