...
> - Adam (方法) --推广了--> SGD (方法)：Adam 推广了 SGD，加入自适应学习率
>
> **学习路径**：MLP 基础 → 激活函数 → **本章** → CNN → RNN/LSTM → 训练数学
>
> **前置知识**：神经元、MLP 结构、损失函数、反向传播、梯度下降（见 [01a-MLP 基础](01a-mlp-basics.md)）
>
> **参考**：
...
在 [01a-MLP 基础](01a-mlp-basics.md) 中你学了 MLP 的基本结构：输入层 → 若干隐藏层（每层做线性变换 + 非线性激活） → 输出层。理论上，只有一个隐藏层的 MLP 就能拟合任意连续函数（万能近似定理）。但实践中，**更深的网络往往比更宽的网络更高效**——用更少的参数就能表达更复杂的函数。
...
在普通 MLP 里，每层的权重矩阵 $W_l$ 各不相同，放大因子各层不同，有一定的相互抵消效果。但在 RNN 里，每个时间步**共享同一个** $W_h$。梯度传播时反复乘同一个矩阵：

$$
\text{梯度} \propto (W_h)^{T-1}
$$

如果 $W_h$ 的最大特征值 $|\lambda_{\max}| = 1.1$，序列长 100 步：$1.1^{99} \approx 12000$——梯度放大一万多倍，参数更新直接崩溃。反过来，$|\lambda_{\max}| = 0.9$ 时：$0.9^{99} \approx 2.6 \times 10^{-5}$——网络完全无法学到第 1 个词和第 100 个词之间的依赖关系。

这就是为什么基础 RNN 在实践中无法处理长序列（详见 [04-rnn-lstm.md](04-rnn-lstm.md) 第 3 节）。

### 1.4 解决方案总览

| 问题 | 解决方案 | 核心原理 |
|------|---------|---------|
| 梯度爆炸 | **Gradient Clipping**（梯度裁剪） | 梯度范数超过阈值时按比例缩小，简单粗暴但有效 |
| 梯度消失（MLP） | **残差连接** | $\mathbf{h} = f(\mathbf{x}) + \mathbf{x}$，梯度传播多了 $+\mathbf{I}$ 项，等于给梯度开了一条不经过连乘的"高速公路"（见本章第 4 节） |
| 梯度消失（RNN） | **LSTM 的细胞状态** | 用**加法**更新 $\mathbf{c}_t = \mathbf{f}_t \odot \mathbf{c}_{t-1} + \ldots$，梯度乘的是可学习的遗忘门（可接近 1），而不是固定的 $W_h$（见 [04-rnn-lstm.md](04-rnn-lstm.md) 第 4 节） |
| 方差不稳定 | **Xavier / He 初始化** | 让每层输出方差保持稳定，不逐层放大或缩小（见本章第 2 节） |

后续章节会逐一展开这些方案。

### 1.5 和激活函数的关系

> 激活函数的完整讲解见 [01b-激活函数](01b-activation-functions.md)。这里只讲它和梯度问题的关系。
...
**ReLU**（Rectified Linear Unit）$f(x) = \max(0, x)$ 的导数是 0 或 1，不会像 Sigmoid 那样持续压缩梯度，因此在一定程度上缓解了梯度消失。但仅靠换激活函数是不够的——我们还需要更多技术（初始化、残差连接等，见后续章节）。
