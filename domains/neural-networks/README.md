...
```
前置：LLM/02 基础（神经元、MLP、损失函数、反向传播、梯度下降）
         ↓
    深入 MLP（初始化、BatchNorm、残差连接、优化器）

  ┌─────── D1 基础架构 ─────────────────────────────────────────┐
  │                                                             │
  │  MLP ──→ CNN ──→ RNN/LSTM ──→ Transformer ──→ GNN         │
  │   │    (空间局部性) (时序依赖)   (注意力)    (图结构)       │
  │   │                                                         │
  │   └─ 统一视角：每种架构 = 不同的 inductive bias             │
  │                                                             │
  │  关键关系：                                                 │
  │  · GCN generalizes CNN to graphs                            │
  │  · Transformer contrasts-with RNN（并行 vs 循环）          │
  ...
  │  监督学习（标准）                                           │
  │    ↓                                                        │
  │  AutoEncoder → VAE（重建 → 变分推断 → 生成）               │
  │    ↓                                                        │
  │  GAN（对抗博弈 → 生成）                                    │
  │    ↓                                                        │
  │  Diffusion Model（去噪 → 生成）                            │
  │                                                             │
  │  关键关系：                                                 │
  │  · VAE generalizes AutoEncoder（点估计 → 概率分布）        │
  │  · GAN contrasts-with VAE（对抗 vs 变分，清晰 vs 稳定）   │
  │  · Diffusion contrasts-with GAN（2022 后取代 GAN）         │
  │                                                             │
  │  注意：范式可以搭配任意 D1 架构                            │
  │  例：DCGAN = CNN + GAN，VQ-VAE = CNN + VAE + 离散化        │
  └─────────────────────────────────────────────────────────────┘

  ┌─────── D3 训练数学 ─────────────────────────────────────────┐
  │                                                             │
  │  优化理论（损失面、SGD 收敛性、Adam）                      │
  │    ↓                                                        │
  │  初始化与归一化（Xavier/He、BatchNorm/LayerNorm）          │
  │    ↓                                                        │
  │  正则化（L2、Dropout、数据增强）                           │
  │    ↓                                                        │
  │  泛化理论（VC 维、PAC、Rademacher → double descent）       │
  └─────────────────────────────────────────────────────────────┘
  ...
  ┌─────── D5 前沿理论 ─────────────────────────────────────────┐
  │                                                             │
  │  Neural Scaling Laws · Double Descent · NTK                │
  │  Mechanistic Interpretability · NAS                        │
  └─────────────────────────────────────────────────────────────┘
```
